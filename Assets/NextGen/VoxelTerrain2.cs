...
{
    public class VoxelTerrain2 : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if ( Input.GetKey( KeyCode.RightArrow ) ){ move += Vector2.right * 10.0f * Time.deltaTime; }
            else if ( Input.GetKey( KeyCode.LeftArrow ) ){ move += Vector2.left * 10.0f * Time.deltaTime; }
            if ( Input.GetKey( KeyCode.UpArrow ) ) { move += Vector2.up * 10.0f * Time.deltaTime; }
            else if ( Input.GetKey( KeyCode.DownArrow ) ) { move += Vector2.down * 10.0f * Time.deltaTime; }

            Slide( m_renderWindow.position + move );
        }
        ...
        public void Initialize( Vector2 position, Vector2 windowSize, float chunkWorldSize )
        {
            ...
            IntRect rect = m_renderWindow.GetWindow();
            m_windowSize = rect.size;

            m_chunks = new VoxelChunk2[ m_windowSize.x * m_windowSize.y ];
            ...
            for( int y = 0; y < m_windowSize.y; y++ )
            {
                ...
                {
                    int cx = rect.origin.x + x;
                    int cy = rect.origin.y + y;

                    VoxelChunk2 chunk = VoxelChunk2.Create( transform, string.Format( "[{0},{1}]", cx, cy ), m_settings, m_chunkSize + 1, m_chunkSize + 1 );
        ...
        public void SetValue( int x, int y, VoxelData val )
        {
            ...
            if ( xx >= rect.origin.x && xx < rect.origin.x + rect.size.x )
            {
                if ( yy >= rect.origin.y && yy < rect.origin.y + rect.size.y )
                {
                    m_dirty[ yy * m_windowSize.x + xx ] = true;
                }
            }
        }
        ...
        void LateUpdate()
        {
            ...
            for( int y = 0; y < m_windowSize.y; y++ )
            {
                for( int x = 0; x < m_windowSize.x; x++ )
                {
                    ...
                    {
                        int ox = ( rect.origin.x + ( x + ( m_windowSize.x - m_cyclicHead.x ) ) % m_windowSize.x ) * m_chunkSize;
                        int oy = ( rect.origin.y + ( y + ( m_windowSize.y - m_cyclicHead.y ) ) % m_windowSize.y ) * m_chunkSize;
                        ...
                        VoxelChunk2 chunk = m_chunks[ y * m_windowSize.x + x ];
        ...
        private void ShiftHorizontal( int amount )
        {
            ...
            amount = System.Math.Min( System.Math.Abs( amount ), rect.size.x );
            ...
            if( direction > 0 )
            {
                for ( int y = 0; y < rect.size.y; y++ )
                {
                    for ( int x = 0; x < amount; x++ )
                    {
                        ...
                        VoxelChunk2 chunk = m_chunks[ cindex ];

                        int cx = rect.origin.x + rect.size.x - x - 1;
                        int cy = rect.origin.y + y;
                        float chunkX = cx * m_settings.voxelSize * m_chunkSize;
                        float chunkY = cy * m_settings.voxelSize * m_chunkSize;
        ...
        private void ShiftVertical( int amount )
        {
            ...
            int direction = System.Math.Sign( amount );
            amount = System.Math.Min( System.Math.Abs( amount ), rect.size.y );
            ...
            if( direction > 0 )
            {
                for ( int y = 0; y < amount; y++ )
                {
                    for ( int x = 0; x < rect.size.x; x++ )
                    {
                        ...
                        int cx = rect.origin.x + x;
                        int cy = rect.origin.y + rect.size.y - y - 1;
                        float chunkX = cx * m_settings.voxelSize * m_chunkSize;
                        float chunkY = cy * m_settings.voxelSize * m_chunkSize;
